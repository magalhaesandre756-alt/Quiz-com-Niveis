...
int questoes(void) {
    ...
    if (arquivo == NULL) {
        printf("Erro ao abrir o arquivo 'perguntas.txt'.\n");
        printf("Verifique se o arquivo existe e esta na MESMA pasta do programa,\n");
        printf("e se o nome esta exatamente em minusculas: perguntas.txt\n");
        pausa();
        return 0;
    }
...
char lerRespostaComTempo(int segundos) {
    ...
    while (1) {
        ...
        if (temporestante != ultimotempo) {
            printf("\rTempo restante: %2d s | Digite A, B, C ou D: ", temporestante);
            ultimotempo = temporestante;
        }
